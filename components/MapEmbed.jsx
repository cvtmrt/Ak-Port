...
export function MapEmbed({ lat, lng, title = "Konum", query, embedSrc, zoom = 14, className = "" }) {
  const q = query || `${lat},${lng}`;
  const src = embedSrc || `https://maps.google.com/maps?q=${encodeURIComponent(q)}&z=${zoom}&output=embed`;
  return (
