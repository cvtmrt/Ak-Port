...
export function localBusinessJsonLd({ ratingSummary, reviews } = {}) {
  const data = {
...
export function productJsonLd(p) {
  return {
    ...
    "@type": "Product",
    name: p.name,
    image: abs(productImage(p)),
    description: p.description || p.shortDesc,
    sku: p.productCode || p.slug,
...
export function blogPostingJsonLd(post) {
  return {
    ...
    publisher: {
      "@type": "Organization",
      name: site.name,
      logo: { "@type": "ImageObject", url: abs("/images/og-default.png") },
    },
